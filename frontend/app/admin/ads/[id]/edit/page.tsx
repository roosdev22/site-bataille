...
import { ArrowLeft, Save, Plus, CheckCircle, Trash2, Loader2 } from "lucide-react";
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3 py-2">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Société <span className="text-red-500">*</span></Label>
              <Input className="text-sm h-8" placeholder="Nom de la société"
                value={form.name} onChange={e => setField("name", e.target.value)} />
              {errors.name && <p className="text-[11px] text-red-500">{errors.name}</p>}
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Contact <span className="text-red-500">*</span></Label>
          ...
          <div className="space-y-1.5">
            ...
            <Input className="text-sm h-8" placeholder="https://www.societe.com"
        ...
        <DialogFooter className="gap-2">
          <Button variant="outline" size="sm" className="text-xs" onClick={onClose}>Annuler</Button>
...
export default function EditAdPage() {
  ...
  // ── Charger les annonceurs + les données de la pub ──
  useEffect(() => {
    if (!adId) return;
    ...
    Promise.all([
      ...
      .then(([advRes, adRes]) => {
        setAdvertisers(advRes.data.results ?? advRes.data);
        
        const ad = adRes.data;
        setFormState({
          advertiser:      ad.advertiser || ad.advertiser_id || "",
          title:           ad.title || "",
          destination_url: ad.destination_url || "",
          alt_text:        ad.alt_text || "",
          format:          ad.format || "banner_top",
          target_category: ad.target_category || ad.category || "all",
          ...
          max_impressions: ad.max_impressions ? String(ad.max_impressions) : "",
          max_clicks:      ad.max_clicks ? String(ad.max_clicks) : "",
        });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await api.put(`/admin/ads/${adId}/`, body);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[300px]">
        <Loader2 size={24} className="animate-spin text-[#1c1c2e]" />
      </div>
    );
  }
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-semibold text-[#1c1c2e]">Modifier la publicité</h1>
        ...
        <button
          ...
          disabled={deleting}
          className="flex items-center gap-1.5 text-[11px] text-red-600 hover:bg-red-50 px-3 py-1.5 rounded-md transition-colors disabled:opacity-50"
        >
          <Trash2 size={13} />
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between bg-gray-50 rounded-lg px-4 py-3">
          <div>
            <p className="text-xs font-medium text-gray-700">Statut de la publicité</p>
            <p className="text-[11px] text-gray-400">
              {isActive ? "La publicité est en cours de diffusion" : "La publicité est en pause"}
            </p>
          </div>
          <button
          ...
          >
            {toggleLoading ? (
              <Loader2 size={12} className="animate-spin text-white ml-0.5" />
            ) : (
              <span
        ...
        {/* ── URL destination ── */}
        <div className="space-y-1.5">
          <Label className="text-xs font-medium">URL de destination <span className="text-red-500">*</span></Label>
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-[10px] text-gray-400">Plus le chiffre est élevé, plus la pub est prioritaire.</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            <Label className="text-xs font-medium">Date de fin <span className="text-gray-400">(optionnel)</span></Label>
            <Input type="datetime-local" className="text-sm"
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <Label className="text-xs font-medium">Impressions max <span className="text-gray-400">(optionnel)</span></Label>
            <Input type="number" min="1" className="text-sm"
              value={form.max_impressions} onChange={(e) => setField("max_impressions", e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs font-medium">Clics max <span className="text-gray-400">(optionnel)</span></Label>
            <Input type="number" min="1" className="text-sm"
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
          ...
          <Button size="sm" className="text-xs bg-[#1c1c2e] hover:bg-[#1c1c2e]/90 text-white gap-1.5"
            ...
            <Save size={13} /> {saving ? "Enregistrement…" : "Enregistrer les modifications"}
          </Button>
        </div>
      </div>
