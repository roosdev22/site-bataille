...
const Icons = {
  ...
  ImageIcon: () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect width="18" height="18" x="3" y="3" rx="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>,
};
...
export default function NewPostPage() {
  ...
  async function handleSubmit(publishDirectly = false) {
    ...
    try {
      ...
      const { data: created } = await api.post("/admin/posts/", body);
      if (publishDirectly && created?.id) {
        await api.post(`/admin/posts/${created.id}/publish/`, {});
      }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-4">
        <Link href="/admin/posts" className="w-9 h-9 rounded-xl border border-gray-200 flex items-center justify-center
      ...
      <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm space-y-6">
        ...
        <div className="space-y-2">
          ...
          {coverPreview ? (
            <div className="relative rounded-2xl overflow-hidden border-2 border-gray-100 group">
              ...
              <div className="absolute bottom-3 left-3 px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-xl text-[11px] text-gray-600 font-medium">
                {coverImage?.name}
              </div>
            </div>
          ) : (
          ...
          <p className="text-[10px] text-gray-400">Format recommandé : 1200×630px</p>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            <p className="text-[10px] text-gray-400">
              💡 <code className="bg-gray-100 px-1.5 py-0.5 rounded-md text-[10px]">![alt](url)</code> pour insérer une image
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map(tag => (
                <button key={tag.id} type="button" onClick={() => toggleTag(tag.id)}
                  className={`px-4 py-2 rounded-2xl text-[12px] font-medium border-2 transition-all duration-200
                    ${form.tags.includes(tag.id)
                      ? "bg-[#1c1c2e] text-white border-[#1c1c2e] shadow-lg shadow-[#1c1c2e]/10"
                      : "bg-white text-gray-500 border-gray-100 hover:border-gray-300 hover:text-[#1c1c2e]"
                    }`}>
                  {tag.name}
                </button>
              ))}
        ...
        <details className="group">
          <summary className="text-[11px] font-bold text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-600 transition-colors select-none">
            SEO (optionnel)
          </summary>
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
          ...
          <Button size="sm" className="text-xs gap-2 bg-[#1c1c2e] hover:bg-[#2a2a3e] text-white rounded-2xl h-10 px-5 font-semibold
            shadow-lg shadow-[#1c1c2e]/10 transition-all"
